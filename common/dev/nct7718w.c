...
uint8_t nct7718w_init(uint8_t sensor_num)
{
	...
	nct7718w_init_arg *init_arg =
		(nct7718w_init_arg *)sensor_config[sensor_config_index_map[sensor_num]].init_args;
	if (init_arg == NULL) {
	...
	if (ret != 0) {
		LOG_ERR("Failed to initailize RT1 High Alert temperature, ret: %d", ret);
		return SENSOR_INIT_UNSPECIFIED_ERROR;
	}
	...
	ret = i2c_master_write(&msg, retry);
	if (ret != 0) {
		LOG_ERR("Failed to initailize RT filter and Alert mode, ret: %d", ret);
		return SENSOR_INIT_UNSPECIFIED_ERROR;
	}
