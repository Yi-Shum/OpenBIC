...
/* Initialization status for spi1_cs0, spi1_cs1, spi2_cs0, spi2_cs1*/
static bool isInitialized[4] = { 0 };
...
uint8_t get_fw_sha256(uint8_t *msg_buf, uint32_t offset, uint32_t length, uint8_t flash_position)
{
	...
	if (!isInitialized[flash_position - 2]) {
		...
		isInitialized[flash_position - 2] = true;
	}
