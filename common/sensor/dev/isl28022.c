...
static isl28022_priv_data *get_priv_data_by_idx(uint8_t idx)
{
	sys_snode_t *node;
...
uint8_t isl28022_init(uint8_t sensor_num)
{
	...
	/* if device has been initialized, get the priv_data and return success */
	if (init_arg->is_init) {
		sensor_config[SensorNum_SensorCfg_map[sensor_num]].priv_data =
			get_priv_data_by_idx(init_arg->index);
		return SENSOR_INIT_SUCCESS;
	}

	/* check if private data of current index is exist */
	if (get_priv_data_by_idx(init_arg->index)) {
		printk("isl28022_init: private data of index %d already exist\n", init_arg->index);
		return SENSOR_INIT_UNSPECIFIED_ERROR;
	}
	...
	new_priv_data->index = init_arg->index;
