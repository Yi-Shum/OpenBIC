...
__weak void OEM_1S_APML_READ(ipmi_msg *msg)
{
	...
	switch (msg->data[0]) {
	case 0x00: /* RMI */
		if (RMI_read(APML_I2C_BUS, SB_RMI_ADDR, msg->data[1], &read_data)) {
			msg->completion_code = CC_UNSPECIFIED_ERROR;
		} else {
		...
		break;
	case 0x01: /* TSI */
		if (TSI_read(APML_I2C_BUS, SB_TSI_ADDR, msg->data[1], &read_data)) {
			msg->completion_code = CC_UNSPECIFIED_ERROR;
		} else {
			msg->data[0] = read_data;
			msg->data_len = 1;
...
__weak void OEM_1S_APML_WRITE(ipmi_msg *msg)
{
	...
	switch (msg->data[0]) {
	case 0x00: /* RMI */
		if (RMI_write(APML_I2C_BUS, SB_RMI_ADDR, msg->data[1], msg->data[2])) {
			msg->completion_code = CC_UNSPECIFIED_ERROR;
		} else {
			msg->completion_code = CC_SUCCESS;
		}
		break;
	case 0x01: /* TSI */
		if (TSI_write(APML_I2C_BUS, SB_TSI_ADDR, msg->data[1], msg->data[2])) {
			msg->completion_code = CC_UNSPECIFIED_ERROR;
		} else {
			msg->completion_code = CC_SUCCESS;
		}
...
__weak void OEM_1S_SEND_APML_REQUEST(ipmi_msg *msg)
{
	...
	apml_data.bus = APML_I2C_BUS;
	apml_data.target_addr = SB_RMI_ADDR;
	apml_data.cb_fn = request_apml_callback_fn;
