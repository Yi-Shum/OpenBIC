...
void apml_report_fail_cb(apml_msg *msg)
{
	if ((msg == NULL) || (msg->ptr_arg == NULL)) {
		printf("[%s] msg passed in as NULL.\n", __func__);
		return;
	}
	sensor_cfg *cfg = (sensor_cfg *)msg->ptr_arg;
	printf("[%s] Failed to report DIMM power/temperature to CPU, sensor number 0x%x.\n",
	       __func__, cfg->num);
}
...
void apml_report_result_check(apml_msg *msg)
{
	...
	if (rddata->error_code != SBRMI_MAILBOX_NO_ERR) {
		printf("[%s] Error when report DIMM power/temperature, error code %d, sensor number 0x%x.\n",
		       __func__, rddata->error_code, cfg->num);
	}
}
...
bool post_ddr5_pwr_read(uint8_t sensor_num, void *args, int *reading)
{
	...
	// printf("[%s] cmd 0x%02x data 0x%02x 0x%02x 0x%02x 0x%02x\n", __func__, wrdata->command,
	//        wrdata->data_in[0], wrdata->data_in[1], wrdata->data_in[2], wrdata->data_in[3]);
