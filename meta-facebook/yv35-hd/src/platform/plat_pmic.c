	...
	while (1) {
		...
		for (dimm_id = 0; dimm_id < MAX_COUNT_DIMM; dimm_id++) {
			...
			if (i2c_master_read(&msg, retry)) {
				LOG_ERR("Failed to get PMIC error, dimm ID %d bus %d addr 0x%x",
					dimm_id, msg.bus, msg.target_addr);
				continue;
			}

			// Compare error pattern, if status change add SEL to BMC and update record
			ret = compare_pmic_error(dimm_id, msg.data);
			if (ret) {
				continue;
			}
		}
