...
#define name_gpioB                                                                                 \
	gpio_name_to_num(PVDDCR_CPU1_BIC_OCP_N) gpio_name_to_num(RST_USB_HUB_R_N)                  \
		gpio_name_to_num(P3V_BAT_SCALED_EN_R) gpio_name_to_num(HDT_BIC_TRST_R_N)           \
			gpio_name_to_num(Reserve_GPIOB4) gpio_name_to_num(FM_CPU_BIC_SLP_S5_N)     \
...
#define name_gpioC                                                                                 \
	gpio_name_to_num(FM_HSC_TIMER) gpio_name_to_num(IRQ_SMB_IO_LVC3_STBY_ALRT_N)               \
		gpio_name_to_num(PVDDCR_CPU1_PMALERT_N) gpio_name_to_num(Reserve_GPIOC3)           \
...
#define name_gpioD                                                                                 \
	gpio_name_to_num(PWRBTN_R1_N) gpio_name_to_num(RST_BMC_R_N)                                \
		gpio_name_to_num(HDT_BIC_DBREQ_R_N) gpio_name_to_num(BMC_READY)                    \
			gpio_name_to_num(BIC_READY) gpio_name_to_num(FM_SOL_UART_CH_SEL_R)         \
				gpio_name_to_num(Reserve_GPIOD6) gpio_name_to_num(Reserve_GPIOD7)
#define name_gpioE                                                                                 \
	gpio_name_to_num(PWRGD_CPU_LVC3) gpio_name_to_num(CPU_ERROR_BIC_LVC3_R_N)                  \
		gpio_name_to_num(PVDD11_S3_PMALERT_N) gpio_name_to_num(IRQ_HSC_ALERT1_N)           \
			gpio_name_to_num(SMB_SENSOR_LVC3_ALERT_N) gpio_name_to_num(Reserve_GPIOE5) \
				gpio_name_to_num(SYS_PWRBTN_BIC_N)                                 \
					gpio_name_to_num(RST_PLTRST_BIC_N)
#define name_gpioF                                                                                 \
	gpio_name_to_num(CPU_SMERR_BIC_N) gpio_name_to_num(IRQ_HSC_ALERT2_N)                       \
		gpio_name_to_num(Reserve_GPIOF2) gpio_name_to_num(FM_BMC_DEBUG_ENABLE_N)           \
...
#define name_gpioL                                                                                 \
	gpio_name_to_num(Reserve_GPIOL0) gpio_name_to_num(Reserve_GPIOL1)                          \
		gpio_name_to_num(BIC_JTAG_MUX_SEL) gpio_name_to_num(BOARD_ID2)                     \
			gpio_name_to_num(PVDDCR_CPU0_PMALERT_N) gpio_name_to_num(Reserve_GPIOL5)   \
